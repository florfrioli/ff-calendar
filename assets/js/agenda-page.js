            ...
            obtenerTurnoActivo().then((turno) => {
                putTurnoNuevoEstado(turno._id, respuesta)
                    ...
                    .catch((err) => {
                        console.log(err);
                        new Alerta(
                            "ERROR - El turno no pudo actualizarse: " + err,
                            "Actualizar turno"
                        ).mostrar();
                        cargarCalendario(opcionEspecialista);
                    });
            ...
            cargarEspecialistasExistentes(ADMIN)
                .then(() => {
                    document.getElementsByClassName("modal-title")[0].innerText =
                        ...
                        ":" +
                        minutos;
                    recolectarPacienteyEspecialista().then((respuesta) => {
                        let turnoNuevo = crearObjetoTurno(datoFechayHora, respuesta);
                        postTurno(turnoNuevo)
                            .then((resp) => {
                                let calendarioActual = document.getElementById("filtro-agenda")
                                    .value;
                                cargarCalendario(calendarioActual);
                                new Alerta(
                                    "El turno fue creado con éxito!",
                                    "Nuevo Turno"
                                ).mostrar();
                            })
                            .catch((err) => {
...
function cargarInfoTurnoEnModal(unPaciente, evento) {
    ...
    document.getElementById(
        "obraSocial-turno"
    ).value = unPaciente.obraSocial.toUpperCase();
    document.getElementById(
        "estado"
    ).value = evento.extendedProps.estado.toUpperCase();
}
...
function obtenerTurnoActivo() {
    return new Promise(function(resolve, reject) {
        let fecha = document
            .getElementById("dato-fecha-turno")
            .innerText.split("Turno: ", 2)[1];
...
function cargarPacientesExistentes() {
    return new Promise(function(resolve, reject) {
        getAllPacientes()
            .then((pacientes) => {
                if (llenarSelectConOpciones(pacientes, "buscadorPacientes"))
                    resolve("Pacientes cargados");
            })
            .catch((err) => {
                console.log(err);
...
function cargarEspecialistasExistentes(permiso) {
    return new Promise(function(resolve, reject) {
        if (permiso == "true") {
            ...
            getAllEspecialistas()
                .then((especialistas) => {
                    if (llenarSelectConOpciones(especialistas, "buscadorEspecialistas"))
                        resolve("Especialistas cargados.");
                })
                .catch((err) => {
                    console.log(err);
            ...
            getUserLogeado()
                .then((especialista) => {
                    if (
                        seleccionarEspecialistaActivo(especialista, "buscadorEspecialistas")
                    )
                        resolve("Especialistas cargados.");
                })
                .catch((err) => {
...
function cargarFiltroEspecialistas(permiso) {
    ...
    if (permiso == "true") {
        ...
        getUserLogeado()
            .then((especialista) => {
                if (seleccionarEspecialistaActivo(especialista, "filtro-agenda"))
                    cargarCalendario(especialista.dni);
            })
            .catch((err) => console.log(err));
    }
}
